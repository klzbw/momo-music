...
Linux 桌面用户通常技术能力较强，但也期望应用能与桌面环境深度集成。momo-music 在 Linux 上的核心集成点是 **MPRIS**（Media Player Remote Interfacing Specification），它是 Linux 桌面统一媒体控制的标准。

## 常见问题

### MPRIS 不工作

**症状**：键盘上的多媒体键（播放/暂停/下一首）无法控制 momo-music。
...
1. 确认系统支持 MPRIS：
   ```bash
   # 运行后测试媒体键
   qdbus org.mpris.MediaPlayer2.momomusic /org/mpris/MediaPlayer2 org.mpris.MediaPlayer2.Player.PlayPause
   ```
2. 重启 momo-music
...
momo-music 在 GNOME 桌面上可以通过以下扩展在顶栏显示歌词和控制信息：
